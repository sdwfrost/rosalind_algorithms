...
def outputLCSLong(backtrack,v,i,j,out,longer):
    done = False
    printed = False
    ilast = i+1
    jlast = j+1
    ...
    return out
def outputLCSShort(backtrack,v,i,j,out,longer):
    done = False
    printed = False
...
out = []
longer  = True
outputLCSLong(backtrack,s2,len(s1),len(s2),out,longer)
print ''.join(reversed(out))
longer = False
out = []
outputLCSShort(backtrack,s1,len(s1),len(s2),out,longer)
