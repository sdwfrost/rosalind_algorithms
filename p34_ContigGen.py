...
###############  Eulerian Cycle  #############################
def gecy(gnodes,starts,nodeStats):
    more = True
    nodes = deepcopy(gnodes)
    cyc = []
    contigs = []
    for start in starts:
        pos = start
        ...
        for i in range(length):
            ...
            while more:
                ...
                try:
                    try:
                        #print nodes[pos]
                        if (nodeStats[pos][1] != 1 or nodeStats[pos][0] > 1) and pos != start:
                            more = False
                            contigs.append(cyc)
                            cyc = []
                            continue
...
def nonBranching(nodes):
    ...
    for node,stats in nodeStats.iteritems():
        if stats[0] == stats[1] and stats[0] == 1:
            odd.append(node)
    return nodeStats

def genContigs(nodes):
    contigs = []
    nodeStats = nonBranching(nodes)
    starts = []

    for n,v in nodeStats.iteritems():
        if v[0] == 0 and v[1] >= 1:
            starts.append(n)
        elif v[1] > 1:
            starts.append(n)
            ...
            starts.append(n)
    contigs = gecy(nodes,starts,nodeStats)
...
def geulerianPath(nodes,nodeStats):
    starts = []
...
mers = open(sys.argv[1]).read().strip().split()

nodes = deBrujnFast(mers)

contigs = genContigs(nodes)
